...
import { githubRepos as staticRepos, GithubRepo } from "@/data/github-repos";

/** "3 days ago" from an ISO date — computed at render, never frozen. */
...
/**
 * FIX: stars/forks/"updated X ago" were hardcoded strings that could
 * never change. This hook renders the static data instantly (so nothing
 * flashes or waits), then overlays real numbers from /api/github when
 * they arrive. If the fetch fails, the static data simply stays.
 */
function useLiveRepos(): GithubRepo[] {
  const [repos, setRepos] = React.useState<GithubRepo[]>(staticRepos);

  React.useEffect(() => {
    let cancelled = false;
    fetch("/api/github")
      .then((r) => r.json())
      .then((json: { ok: boolean; repos?: LiveRepo[] }) => {
        if (cancelled || !json.ok || !json.repos) return;
        const live = new Map(json.repos.map((r) => [r.name.toLowerCase(), r]));
        setRepos(
          staticRepos.map((r) => {
            const fresh = live.get(r.name.toLowerCase());
            if (!fresh) return r;
            return {
              ...r,
              ...
              stars: fresh.stars,
              forks: fresh.forks,
              updatedAt: timeAgo(fresh.pushedAt),
            };
          })
        );
      })
      .catch(() => {
        /* offline or blocked — static fallback already on screen */
      });
    return () => {
      cancelled = true;
    };
...
export function GithubPage() {
  ...
  const githubRepos = useLiveRepos();
...
function AnimatedStatCard({
  ...
  // FIX: same "0+" server-render bug as the home page — real value first,
  // count-up only as an enhancement.
